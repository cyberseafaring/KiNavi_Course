...
![](images/ed98cda7bba27db09e125cdd4df1e616e7a74a68a7a21c3d36089031be5507ad.jpg)  
...
![](images/4d517123a5415620407b6b3d450b16eae0dc5c0add045f8d1fe5fcd18984b9f5.jpg)  
...
![](images/7c8ab4f31a95ae8efdc6ec36a016b97b538838a45e2f818a988e4087d359fc83.jpg)  
...
![](images/9da5495a71151aa624e63db2fbe1d3568bdf3650a85c166725d4c771d9678134.jpg)  
...
![](images/0b3a4f04bcaea0a71002cdbb86dcdfb4722fc937d30621e345d497a628483a11.jpg)  
...
![](images/c9bb06bd6f4209504b3d9ae78c1d00c91e6b2ec826d6b6fdad12226e3fa8baff.jpg)  
...
![](images/d68ff37110a3cfd21c0ae8d79053ef4cbf5b24fc7a6c050d23c0adbbaf535439.jpg)  
...
![](images/91f3f3b2f88130fe467367a6d92834b078def96916793db79f8342ae0f4d2e9f.jpg)  
...
![](images/b74f826194122e39b5a2ff5fbd6d1d4cd18d60f0c66aab5deea1406bb6d342c8.jpg)  
...
![](images/35782728a8f4862f4eac9d2ee37beb41ec5d0d178206405666b00f70f05c48f5.jpg)  
...
【解读】
该图展示了相对运动的概念，即两个或多个物体之间的相对位置和运动关系。在这个例子中，图像中可能显示了一个背景和一个前景物体，它们之间的相对运动被呈现在视觉上。这可以帮助研究人员或学习者理解和分析相对运动的原理和特征。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Relative-motion | 相对运动 | 指两个或多个物体之间的相对位置和运动关系。|
| Presentation | 呈现 | 指以某种方式展示或表现信息或概念。|
| Figure | 图 | 指插图、图表或其他形式的视觉辅助工具。|



$(T)$ at a speed of 10 knots through the water. iii. Vessel C which is located 5 NM due north of vessel $A$ and is on a steady heading of  

$180^{\circ}$ $(T)$ at a speed of 5 knots through the water. iv. A large navigational buoy $L$ which is anchored and therefore, for the purposes of this example, can be considered to be stationary over the ground. Its position is 7 NM due west of vessel $A$ .  
...
【翻译】
最初，浮标与静止船只之间的东西向距离为7海里。由于潮汐使得静止船只以每小时4节的速度朝着浮标移动，因此到上午10点30分，这个距离就会减少到5海里。研究图1.17（c）可以看出，浮标的回声在屏幕上移动的方向是观测船只的地面航迹的逆向，其速度等于观测船只在地面上的速度。这一特性被利用在雷达导航中（与避碰相反），各种程序都在第8章中列出。

【解读】
本段主要讨论了浮标、静止船只和观测船只之间的相对运动关系，以及如何利用雷达技术来进行导航。通过分析浮标回声在屏幕上的移动，可以确定观测船只的地面航迹和速度，从而辅助导航。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Buoy | 浮标 | 一种用于海上标志或参考点的漂浮装置 |
| Reciprocal | 逆向 | 指两个方向相反的关系 |
| Ground track | 地面航迹 | 船只在地面上的实际行进路线 |
| Radar | 雷达 | 无线电探测和定位系统，常用于导航和避碰 |



motion or stabilisation mode, simply because the ‘world outside’ is obviously not affected by the settings of the radar.  

【翻译】
运动或稳定模式，只是因为“外部世界”显然不受雷达设置的影响。

【解读】
这段话强调了雷达设置（如运动或稳定模式）与外部环境之间的独立性，指出外部世界不会受到这些设置的影响。这意味着雷达的工作模式主要是为了适应内部系统需求，而不是为了改变或影响外界环境。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| motion | 运动 | 指物体的移动状态或运动模式 |
| stabilisation | 稳定 | 指使系统保持稳定的过程或状态 |
...
【翻译】
很明显，在一段时间之后，观察船只（即原点）的位置会移动到显示屏的边缘。这时，它需要被重置，这可以通过用户干预或由用户设置的自动过程来实现。重置策略在第6.2.6.3节中进行了讨论。

【解读】
这段话主要讲述的是一种观察系统中的坐标原点（origin）会随着时间的推移而变化，导致其位置从中心移到显示屏的边缘，从而需要进行重置操作。这种重置可以是人工触发的，也可以是自动完成的，并且相关的策略会在后续章节中详细讨论。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| origin | 原点 | 指坐标系中的起始点或参考点 |
| observing vessel | 观察船只 | 指进行观察或测量的设备或平台 |
| reset | 重置 | 将系统或参数恢复到初始状态的操作 |
| intervention | 干预 | 人为地干涉或控制某个过程或系统 |
| automatic process | 自动过程 | 由机器或计算机程序自动执行的过程，无需人工干预 |
...
![](images/00c122972885574c8d22cd7ff2a81363b6d0fda928e8806f26c5434495699092.jpg)  
...
【解读】
该图展示了海上稳定化技术在真实运动中的应用，能够减少因波浪或船只运动引起的视觉抖动，提供更平滑、更清晰的观察体验。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| true-motion | 真实运动 | 指的是物体或相机在运动时保持其实际轨迹的状态，不受外界干扰。 |
| sea-stabilized | 海上稳定化 | 一种用于减少海面波浪对视觉或设备影响的技术，常用于航海、航空等领域。 |
| presentation | 呈现、表现 | 指的是信息、数据或者图像以特定的方式被显示或呈现给观众。 |
...
【翻译】
在考虑真实运动海上稳定化呈现的特性时，必须认识到显示的目标运动的准确度完全取决于图片原点运动方向和速度代表观察船舶航向和STW（船速通过水）的准确度。真实运动呈现的质量只与输入数据的质量一样好。

【解读】
本段强调了真实运动海上稳定化呈现技术的关键特征，即其准确性直接依赖于输入数据的精度。这意味着，如果输入数据不够准确，整个系统的表现就会受到影响，因此高质量的输入数据对于获得可靠的真实运动呈现至关重要。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| truemotion | 真实运动 | 一种模拟或呈现物体运动的方式，旨在提供最真实的运动感受 |
| sea-stabilized | 海上稳定化 | 指在海上环境中对图像或设备进行稳定处理，以减少晃动等影响 |
| STW | 船速通过水 | 指船舶在水中的速度，是判断船舶运动状态的一个重要参数 |
...
【翻译】
关于设置演示和错误、不准确性影响的实际步骤分别在第6.2节和第7.9节中进行了介绍。由于所使用的场景中观察船的朝向为北，因此当前使用的方向模式是无关紧要的。需要注意的是，任何方向模式（除了非稳定头向上模式）都可以与真实运动一起使用，无论其朝向如何。

【解读】
本段主要讨论了有关设置演示和处理错误的程序，以及不同方向模式对真实运动的影响。它强调了选择合适的方向模式以获得最佳效果的重要性，同时也指出某些模式可能并不适用。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Practical Procedure | 实际步骤 | 指完成特定任务的具体操作方法 |
| Orientation Mode | 方向模式 | 指设备或系统中物体的方向设定方式 |
| True Motion | 真实运动 | 指按照实际物理规律模拟的运动效果 |
| Heading | 朝向 | 指航海或航空中，船只或飞机的前进方向 |
...
【翻译】
为了创建真实运动演示的场地稳定化，图片的原点会按照观察船只在陆地上的航迹方向和速度移动。在精确的定位系统（如GNSS）出现之前，需要独立测量观察船只的航向、STW（通过水速），以及估计潮流或洋流的方向和速度。这种复杂性现在通常不再必要，因为PPI（计划位置指示器）可以被认为是参照一个固定在地球坐标系上的坐标系统，如GPS和电子导航图表（ENCs）使用的WGS-84。

【解读】
本段主要讨论如何实现真实运动演示中的场地稳定化，以及过去和现在获取相关数据的方法。随着技术的进步，尤其是精确的定位系统的出现，大大简化了这一过程，使得对观察船只的位置和运动参数的测量变得更加容易和准确。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| GNSS | 全球卫星导航系统 | 一种利用卫星实现全球范围内的导航和定位的系统 |
| PPI | 计划位置指示器 | 一种显示预定航线和当前位置的设备或软件 |
| WGS-84 | 世界大地测量系统1984 | 一种国际标准的地球坐标参考系统，广泛用于GPS等领域 |
| STW | 通过水速 | 船只相对于水体的速度 |
...
【翻译】
为了方便比较，我们以与前两个例子相同的情景为参考来说明这个问题。图1.19（b）显示了观察船的PPI在1000时的样子，而图1.19（c）则显示了1030时的回声，以及记录下来的1000时位置的轨迹。可以看到，这个起点移动的方向与航向标志的方向不同。后者表示观察船在任何时候所朝的方向，与潮汐影响无关。

【解读】
本段主要讨论的是如何通过图像来比较和分析观察船的位置和运动情况，特别是指出观察船的实际运动方向可能会受到潮汐等因素的影响，从而与其预期或记录下的航向有所不同。这需要仔细观察和分析图像中的信息，以准确地判断观察船的状态。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| PPI | 平面位置指示器 | 一种用于显示船只位置和周围环境的导航设备 |
| observing vessel | 观察船 | 指进行观察或监测任务的船只 |
| heading marker | 航向标志 | 表示船只当前航向的指示器 |
| tidal influence | 潮汐影响 | 潮汐对海洋或水体的影响，可能导致水位、流速等变化 |
...
![](images/1fee1a5ba3deed9426e41aae255b7c77c65756639ab91a1473fa888ed9d9af8f.jpg)  
...
【解读】
该图展示了真实运动的地面稳定化呈现，这意味着图像或视频中物体的运动是相对于地面的，而不是相对于观察者的。这项技术可以帮助减少晕动症的发生，提高观众对画面内容的理解和跟随能力。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| True-motion | 真实运动 | 指的是物体在图像或视频中的运动方式与其在现实世界中的运动方式一致。 |
| Ground-stabilized | 地面稳定化 | 指的是以地面为参考系，消除相机抖动等因素对图像的影响，使得图像更加稳定清晰。 |
| Presentation | 呈现 | 指的是信息、数据或者图像通过某种形式（如图表、视频等）向观众展示的过程。 |
...
【翻译】
因此，在本次演示中，移动中的船舶回波的运动并不代表它们的航向。了解航向信息对于正确使用雷达以避免碰撞至关重要。由此可见，一般来说，如果不使用跟踪目标（TT）演示，本次演示不适合作为规划避碰策略的依据，因为它可能带来危险性的误导。这一极为重要的话题在第6.9.6节中进一步讨论。如前一节所述，应该理解，无论是北向上还是航程向上的定向，都可以与真实运动一起使用。

【解读】
本段强调了在使用雷达进行避碰时，了解船舶的航向信息的重要性，并指出如果没有跟踪目标的辅助，仅凭借回波的运动可能会产生误导，从而影响安全。同时，也提到了不同定向方式（北向上或航程向上）的应用灵活性。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Heading | 航向 | 指船舶或飞机等的行进方向 |
| Collision Avoidance | 避碰 | 指通过各种手段避免相互碰撞的措施或技术 |
| Tracked Target (TT) | 跟踪目标 | 指在雷达系统中被跟踪监视的目标物体 |
| True Motion | 真实运动 | 指显示物体实际运动状态的雷达显示模式 |
...
【翻译】
在这种模式下，一般可以选择固定观察船只（即观察者的位置）的起始点到显示屏上的任何位置。一开始，这似乎使得显示与1.5.1节中讨论的相对运动呈现方式相同。但是，有一个重要的区别：显示出的目标轨迹将以其原始的地面参考位置显示，而不是作为相对于观察船只的位置。这消除了当起始点移动到整个显示区域时发生的“监视区域”变化的问题，特别是在“向前看”功能方面可能会引发问题。

【解读】
本段主要介绍了一种显示模式下的特性，即允许用户固定观察者位置，并且显示目标的轨迹以地面为参考系，而非相对于观察者。这解决了由于观察者位置变化导致的显示区域变化问题，尤其是在需要预测目标未来位置的应用场景中。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Origin | 起始点/原点 | 指定显示的参考点或坐标系的起点 |
| Relative-motion presentation | 相对运动呈现方式 | 一种显示方式，目标的位置是相对于观察者的 |
| Ground referenced position | 地面参考位置 | 以地面为基准的坐标系中的位置 |
| Surveillance area | 监视区域 | 被监控或跟踪的区域 |
...
【翻译】
在任何特定情况下，呈现方式的选择都会受到多种因素的影响。在某种程度上，决定呈现方式的关键因素是雷达是否主要用于碰撞避免或位置确定与进度监测。如果考虑到一些细微差别，最好是在进一步讨论雷达系统的操作原理、设置和维护演示的实际程序以及使用雷达进行碰撞避免和导航的一般哲学之后（见第6-8章），再详细讨论影响此类选择的因素。然而，在这个阶段，为了比较目的，总结每种呈现方式的主要特征并简要评论其在碰撞避免和导航中的适用性是有用的。

【解读】
本段讨论了雷达呈现方式的选择会受到多种因素的影响，包括雷达的主要用途，如碰撞避免或位置确定等。作者认为，需要在了解雷达系统的基本原理和使用方法后，再深入讨论这些因素的影响。同时，也对不同呈现方式的优缺点进行了初步的比较和评估，以便于读者理解和选择合适的呈现方式。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Collision avoidance | 碰撞避免 | 使用雷达等技术来避免船舶、车辆等相互碰撞 |
| Position fixing | 位置确定 | 确定物体当前所处的位置 |
| Progress monitoring | 进度监测 | 监测和跟踪物体运动的进度和状态 |
| Radar system | 雷达系统 | 利用雷达技术来探测和跟踪目标的系统 |
...
【翻译】
在相对运动显示中，正在移动的目标的回声运动是指观察船只相对于目标的运动。系统地观察这种运动可以轻松地预测目标将经过的距离（最近接近点或CPA）以及目标达到最近接近点的时间（TCPA）。这些信息是衡量近距离情况发展风险的有效措施。没有添加目标跟踪矢量，该显示不直接指示目标船只的航向或速度。因此，相对运动显示直接给出了部分用于避免碰撞所需的信息，但其余部分必须通过推断来确定（见第7章）。

【解读】
本段主要讨论了相对运动显示在海事领域中的应用，特别是在避免碰撞方面的作用。通过分析目标与观察船只之间的相对运动，可以预测目标最接近的点及其到达时间，从而评估近距离情况的风险。这一技术为航海安全提供了重要的参考价值，但也需要结合其他信息进行综合判断。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Relative-motion | 相对运动 | 指两个物体之间的相对位置和运动关系 |
| Closest Point of Approach (CPA) | 最近接近点 | 指两船之间最近的距离点 |
| Time of Closest Point of Approach (TCPA) | 到达最近接近点的时间 | 指目标到达其最近接近点的时间 |
| Collision Avoidance | 避免碰撞 | 指采取措施防止船只之间发生碰撞的行为 |
...
【翻译】
陆地目标在相对运动演示中的回声会描绘出一条轨迹，这条轨迹是观察船只在地面上实际行进路线的倒数。这一特性在监测进度和确定位置时尤其有用，特别是在雷达用于导航而非避碰（见第八章）的时候。

【解读】
这段话主要讨论了雷达技术在导航中的应用，特别是如何利用陆地目标的回声来辅助确定船只的位置和监测其行进情况。这种方法对于确保安全高效的航行具有重要意义。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| reciprocal | 倒数 | 指的是两个数量之间的互为倒数的关系，在这里指的是观察船只的地面轨迹与陆地目标回声轨迹之间的关系 |
| relativemotion | 相对运动 | 指的是两个或多个物体之间的相对运动状态，在这里指的是观察船只与陆地目标之间的相对运动 |
| navigation | 导航 | 指的是通过各种手段和工具来确定位置、方向和速度，以便安全高效地到达目的地的过程 |
| collision avoidance | 避碰 | 指的是为了防止碰撞而采取的措施和策略，在海事领域中尤其重要 |
...
【翻译】
表1.2 演示 # 特征摘要（假设无自动绘图功能）

【解读】
该段落似乎是从一份技术文档或研究论文中提取出来的，描述了一张名为“表1.2”的表格，该表格总结了演示的特征，特别是在没有自动绘图功能可用的情况下。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Presentation | 演示 | 指的是一种展示信息的方式，可能包括文字、图片、图表等。 |
| Feature | 特征 | 在这里指的是演示中的某些属性或元素，如图表、数据展示等。 |
| Automatic Plotting | 自动绘图 | 指的是计算机自动生成图表或图形的能力，无需人工干预。 |




<html><body><table><tr><td>Feature</td><td colspan="3">Presentation</td></tr><tr><td></td><td>Relative-Motion</td><td>True-Motion Sea-Stabilized</td><td>True-Motion Ground-Stabilized</td></tr><tr><td>Ease of assessing target's Directly available CPA/TCPA from trails</td><td></td><td>Resolution required</td><td>Resolution required</td></tr><tr><td>or hand plots Ease of assessing target's Resolution required course, speed and aspect</td><td></td><td>Directly available</td><td>Resolution required — potentially misleading</td></tr><tr><td>Need for additional sensor inputs: course</td><td>No, but course input desirable</td><td>Yes</td><td>Yes</td></tr><tr><td>and speed Need for data on tide setNo and rate or ground</td><td></td><td>No</td><td>Yes</td></tr><tr><td>speed and course Displayed information relative to:</td><td>Observer</td><td>The water</td><td>The ground</td></tr><tr><td>Particular application for collision avoidance/ navigation</td><td>rPartial contribution to collision avoidance data; ideal for parallel indexing</td><td>Partial contribution to collision avoidance data</td><td>Difficult to achieve without Automatic Radar Plotting Aid (ARPA) but if achieved provides stationary map</td></tr><tr><td>Limitations for collision avoidance</td><td>Target heading not directly available</td><td>CPA not directly available</td><td>No collision avoidance data directly available</td></tr><tr><td>Limitations for navigation</td><td>Movement of land echoes may hinder target identification</td><td>Limited movement of land echoes</td><td>None if stabilization effective</td></tr></table></body></html>  
...
【翻译】
真实运动海上稳定化显示方式可以直接获取目标物的航向和速度。在使用雷达进行碰撞避免方面，可以看出相对运动和真实运动海上稳定化显示方式是互补的。真实运动具有额外的优势，即它使得识别目标机动变得非常容易，而且当观察船只进行机动或转向时，目标运动的连续性不会被打断。

【解读】
本段主要讨论了在雷达系统中使用真实运动海上稳定化显示方式的优点，特别是在碰撞避免方面。这种显示方式不仅能直接提供目标物的航向和速度，还能帮助更容易地识别目标物的机动，并且即使观察船只发生变化，也不会影响目标运动的连续性跟踪。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| true-motion | 真实运动 | 指的是以实际的、没有被任何其他运动干扰的方式来表示目标物的运动状态。 |
| sea-stabilized | 海上稳定化 | 是一种技术，用于消除由于观察船只自身运动而引起的干扰，从而得到更加稳定的目标物运动数据。 |
| collision avoidance | 碰撞避免 | 使用雷达等技术来预防船只之间或船只与其他障碍物之间的碰撞。 |
| relative-motion | 相对运动 | 指的是两个或多个物体之间的相对位置和运动关系。 |
| yaw | 转向 | 船只横向旋转的运动。 |
...
【翻译】
表1.2总结了上述演示的特征，这些特征是从传统的手动绘图角度出发或者是在屏幕上观察目标轨迹得出的。

【解读】
本段主要介绍了一张表格（表1.2），它概括性地总结了某种演示或展示的特点。这些特点可以通过两种方式来观察：一种是采用传统的手动绘图方法，另一种是在电子设备的屏幕上直接观察目标物体的运动轨迹。这意味着该表格试图从不同视角来描述和分析某一呈现形式的特征。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Presentation | 演示/展示 | 指的是一种信息、数据或想法的呈现方式 |
| Traditional | 传统的 | 指的是长期以来被广泛接受和使用的方法或方式 |
| Perspective | 视角/观点 | 指的是看待事物的方式或角度 |
| Target trails | 目标轨迹 | 指的是在屏幕上显示的目标物体的移动路径 |
...
【翻译】
现代观察员通常在确定目标数据时会得到ARPA或TT（目标跟踪器）的帮助。这使得上述讨论的大部分变得不再相关，因为连续跟踪和计算能力意味着真实和相对目标数据（向量）可以通过“按一下按钮”来显示。如果查看真实数据（向量），则海面和地面稳定化数据（向量）的显示（如果可用），也可以通过按另一个按钮来改变。这并不减少观察员需要了解这些不同向量的不同特性的需求，但这确实意味着在现代设备上，可以使用较不严格的标准来选择呈现方式。

【解读】
本段主要讨论了现代观察技术如何辅助观察员获取目标数据，并且如何通过简单的操作切换不同的数据显示方式，从而提高观察效率。同时，也强调了观察员仍然需要理解不同类型数据的特性，以便更好地利用这些技术。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| ARPA | 自动雷达plotter分析仪 | 一种自动处理雷达数据的系统 |
| TT | 目标跟踪器 | 跟踪和记录目标移动的设备 |
| Vectors | 向量 | 在此 context 中指目标的运动方向和速度等信息 |
| Stabilized data | 稳定化数据 | 经过滤波或校正后更加准确可靠的数据 |
...
【翻译】
在现代ARPA设备中，显示方式的选择往往取决于观察者希望如何控制显示屏上所显示的区域。相对运动意味着中心点保持不变，而观察者可以选择中心点位于几何中心或偏移，以便更好地观察到可能存在危险或有用的方向。

【解读】
本段主要讨论了现代ARPA（空中雷达预警系统）设备中的显示设置选项。它提到了“相对运动”的概念，这使得观察者能够自定义显示屏上的内容布局，以便更好地监控特定区域或方向。这一功能增强了观察者的灵活性和实用性，使其能够根据实际需要调整显示设置以获得最佳观察效果。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| ARPA | 空中雷达预警系统 | 一种用于航空领域的雷达预警系统 |
| Relative-motion | 相对运动 | 指的是在显示屏上，中心点保持静止，而其他元素相对于中心点进行移动的显示模式 |
| Geometric centre | 几何中心 | 指的是一个图形或物体的中心点，通常是通过数学计算确定的 |
| Offset | 偏移 | 指的是某个元素或物体相对于其正常位置或中心点的偏离或移动 |
...
【翻译】
标准的真实运动选项意味着中心点会随时间在显示屏上移动，因此被覆盖区域减少。这可以看作是一个缺点，尽管现代设备上偶尔将中心点向后重置是一个快速且直接的选项。一个优点是，当船只经过某一特征时，如显眼的海岬，可以保持该特征在显示屏上的位置不变。

【解读】
这段话讨论了航海电子设备中“真实运动”（true motion）模式的特点及其带来的影响。在这种模式下，显示屏上的中心点会随着时间的推移而移动，这可能会使得用户难以跟踪某些特定区域。但同时，这种模式也允许当船只经过某些固定特征（如海岸线上的突出地貌）时，这些特征在显示屏上的位置不会改变，从而为导航提供了一定的便利性。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| true-motion | 真实运动 | 一种模拟船舶实际运动的显示模式 |
| centre spot | 中心点 | 显示屏上的参考点，通常表示船舶当前位置 |
| conspicuous | 显眼的 | 引人注目的，容易被注意到的 |
| headland | 海岬 | 一种沿海岸突出的陆地或岩石地貌 |
...
【翻译】
仍然需要记住，目标轨迹通常由呈现方式来控制，这会影响对目标机动的解释。

【解读】
这段话强调了在分析目标行为时，必须考虑到呈现方式（即如何向观察者展示目标）对目标轨迹的控制作用。这意味着同一个目标可能因为呈现方式不同而表现出不同的行为特征，从而影响观察者的判断和解释。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| Target trails | 目标轨迹 | 指的是目标物体运动或移动的路径或轨道 |
| Presentation | 呈现方式 | 指的是向观察者展示目标的方法或形式 |
| Interpretation | 解释 | 指的是对某事物的理解或说明 |
...
【翻译】
本节中对结合ARPA和TT设施的演示控制现代应用进行了初步探讨，更详细的内容将在第6.9节中展开。

【解读】
这段话提到了使用ARPA（可能是指Advanced Research Projects Agency，即美国高级研究计划局）和TT（可能是指TeleType或其他通信技术）设施来增强演示控制的现代应用，并指出更详细的信息会在后续章节中介绍。这意味着作者正在为后面的章节做铺垫，引导读者去关注这些技术与演示控制的融合带来的创新和可能性。

【词汇表】
| 英文词汇 | 中文翻译 | 解释 |
| --- | --- | --- |
| ARPA | 高级研究计划局 | 美国的一个机构，负责推动尖端技术的研发 |
| TT | 电传打字机/通信技术 | 一种早期的电信设备或通信技术的缩写 |
| Facilities | 设施、设施 | 指为某项活动或服务提供的条件或资源 |
